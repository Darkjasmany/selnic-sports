...
type Props = {
  ...
  onUpdate: (id: string, name: string, disciplineId?: string) => void;
  disciplines: Discipline[];
};

const CategoryTable = ({ categories, isLoading, onDelete, onUpdate, disciplines }: Props) => {
  ...
  const handleStartEdit = (category: Category) => {
    ...
    setEditingDisciplineId(category.disciplineId);
  };
  ...
  const handleSave = (id: string) => {
    if (!editingName.trim() || !editingDisciplineId.trim()) return;
    onUpdate(id, editingName.trim(), editingDisciplineId);
    handleCancelEdit();
  };
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-800">
            ...
            <th className="text-left py-3 px-4 text-slate-400 font-medium hidden sm:table-cell">Disciplina</th>
          ...
          {categories.map((category, index) => (
            ...
            >
              ...
              <td className="py-3 px-4">
                {editingId === category.id ? (
                  <input
                    ...
                    className="h-8 px-2 rounded bg-slate-800 border border-sky-500 text-white outline-none w-full"
                  />
                ) : (
              ...
              <td className="py-3 px-4 hidden sm:table-cell">
                {editingId === category.id ? (
                  <select
                    value={editingDisciplineId}
                    onChange={e => setEditingDisciplineId(e.target.value)}
                    className="h-8 px-2 rounded bg-slate-800 border border-sky-500 text-white text-xs outline-none w-full"
                  >
